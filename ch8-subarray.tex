...
For evern row and column that has a detector that responds in the superconducting state, I fit the resulting noise power spectrum to an equation of the form
\begin{equation}
...
For taking video images, the servo does not need to be run as aggresively. Servo gain $I = \num{20}$ is sufficient for these measurements. (xxx justify this based on measured time constants later).

\begin{figure*}
\includegraphics{drawings/ch8-servo.pdf}
\caption{Plots summarizing beahavior using different servo gains $I$.
  Box plot boxes represent the \SIrange{5}{95}{\percent} quantiless, middle line the median, upper and lower whiskers the maximum/minimum values, across all rows/columns with detectors that respond in the superconducting state.
\textbf{Upper} Plot of \SQUID\ $\tau_{servo}$ vs servo gain $I$.
Servo bandwidth increases with $I$.
At $I \ge \num{60}$, very small $\tau_{servo}$ begin to appear. This indicates either a rolloff above the bandwith of the measurement, or the begining of an unstable servo loop.
...
Values for $\tau_{eff}$, $s_I(0)$ are for detector biased at normal operating conditions of $T_b = \SI{1100}{\milli\kelvin}$ and detector bias of 27000.}
