...
\caption{
  ...
  THe y-intercept at $I_{bias} \delta I = 0$ gives $\tau$, \SI{0.00}{\ms}xxx for this detector.
} 
...
Our approach to dealing with this offset is covered in \sectionref{xxx}.
...
\figref{fig:ch8-bias-step-plots} and \figref{fig:ch8-bias-step-results} show the result of these measurements being taken at a range of bias point for the four detectors on column 0 with good heaters.
...
As the detector is biased deeper into the transition the decrease in current becomes larger, indicating that loop gain increases deeper in the transition (xxx is this an accurate explanation?) 
\textbf{Upper Right}
