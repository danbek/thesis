...
Driving the \TES\ with a sinusoidal $\delta P$ term, holding detector bias constant, leads to the following complicated expression for the detector power-to-current responsivity:
\begin{equation}
xxx
\end{equation}
xxx commentary
...
\eqnref{eqn:bias-step-resp} depends on \Loop and $\beta_I$ in a complicated way. Nevertheless, if the response of a \TES\ to a bias step can be measured with sufficient bandwidth to track the initial fast electrical response, bias steps can be used to measure \Loop and $\beta_I$ by performing non-linear parameter fitting to \eqnref{eqn:bias-step-resp}.
