...
The factor $\mathcal{K}$ depends on the bias point, but high in the transition this dependence is very weak, so that it can be treated as a constant.
...
\begin{tabular}{l l l}
\toprule
Noise Source &  Current Noise & Power-referred Noise \\
\midrule
\TES\ Resistor & $S_{I_{TES}} = 4 k_B T_0 I_0^2 R_0 \xi(I_0) \frac{(1 + (\omega \tau)^2) } {\Loop^2} |s_I(\omega)|^2$  & $4 k_B T_0 I_0^2 R_0 \xi(I_0) \frac{(1 + (\omega \tau)^2)} {\Loop^2}$ \\
Load Resistor & $S_{I_{L}} = 4 k_B T_L I_0^2 R_L \frac{(1 + (\omega \tau_I)^2) (\Loop -1)^2} {\Loop^2} |s_I(\omega)|^2$  & $4 k_B T_L I_0^2 R_L \xi(I_0) \frac{(1 + (\omega \tau_I)^2) (\Loop -1)^2} {\Loop^2}$   \\
Thermal Fluctuation Noise & $S_{TFN} = 4 k_B T_0^2 G F(T_0, T_b) |s_I(\omega)|^2$ & $4 k_B T_0^2 G F(T_0, T_bath)$ \\
\bottomrule
\end{tabular}
\end{table*}
