...
\delta I(t) = - \frac{\delta P}{V_0 \gamma}\frac{\Loop}{\Loop + 1}
                (1 - e^{-t/\tau_{eff}}).
\end{equation}
As the fully normal state is approached, $\tau_{eff}$ approaches $\tau$, so that measuring the time constant very high in the transition will give a mesurement of $\tau$.
