...
\figref{fig:ch4-primary-ray-trace} contains a system ray-trace diagram for the system focused at \SI{16}{\m}.
...
\caption[Parameters of the optical system extracted from \ZEMAX\ simulations]{
  ...
  See \figref{fig:ch4-primary-ray-trace}.
}
...
The feedhorn beam pattern follows the model described above, and is well-approximated over the primary mirror by a Gaussian with \FWHM\ of \ang{9}, or \SI{395}{\mm} in terms of radial distance on the primary.
To calculate a theoretical far-field beam, we can make the simplifying assumption that the primary mirror is illuminated by an electric field with a circularly symmetric Gaussian profile, with a \FWHM\ of $\sqrt{2} \times \SI{464}{\mm} = \SI{560}{\mm}$, where the $\sqrt{2}$ converts from power to electric field.
...
\begin{equation}
  P(\theta) = \left[ \int_{\rho_{in}}^{\rho_{out}} e^{-\frac{1}{2} \frac{\rho^2}{\SI{560}{\mm}}} J_0(\frac{2 \pi}{\lambda} \rho \sin(\theta)) \rho d\rho \right]^2,
\end{equation}
where $J_0$ is a Bessel function of the first kind, $\lambda$ is the central wavelength of the optical band, and $\rho_{in}$ and $\rho_{out}$ are taken from \tableref{tab:ch4-zemax-parms}.
...
\chapterref{c:imaging} present maps of both a \SI{0.2}{\in} and \SI{1.791}{\cm} sources.
...
\caption[Beam patterns]{
  ...
  Also plotted is the best-fit Gaussian within the blue lines, which has a \FWHM\ beam width of \ang{9}.
  ...
  The side-lobe is caused by the secondary mirror, and is \abt{\SI{5}{\percent}} high.
}
\label{fig:ch4-beams}
\end{figure*}
...
% Removing this because it doesn't add anything to the discussion,
% although it *is* good to keep in mind
% 
...
Rather, the \Imager\ is viewing a \abt{\SI{300}{\K}} source that is attenuated by a series of lossy elements, each of which is at non-zero temperature and so emits power itself.
...
\begin{tabular}{ccccccc}
% The content of this table is producted by dissertation/calc_optical_loading()
\toprule 
  Component  & 
  \specialcell{Temperature \\ (\si{\K})} & 
  Efficiency & 
...
Each detector output is coupled into the input coil of its own ``1st-stage'' \SQUID (\SQ1).
...
To linearize the \SQUID\ amplifier chain, feedback in the form of magnetic flux is applied to the \SQ1 \SQUIDs.
...
The exceptions are the feedback control parameters, discussed in the next section, and the parameters that control the readout rate of the system, listed in \tableref{tab:ch4-mce-parms}.
...
As discussed in \sectionref{sec:ch6-aliasing}, this raises the total amount of noise in the \Imager's detectors by \SI{16}{\percent} on average.
...
\figref{fig:ch6-servo-tau-est} shows the impact of the servo roll-off on estimation of time constants from single-pole response functions.
...
\begin{figure*}
\includegraphics{drawings/ch6-servo-tau-est.pdf}
...
\figref{fig:ch6-servo} shows the resulting white noise levels and $\tau_{servo}$ values for a range of servo gain values $I_{FB}$.
...
As shown in \figref{fig:ch6-teff-si-pred}, most detectors have $\tau_{eff}$ close to \SI{1.25}{\ms}, with only a few slower than this.
...
\begin{figure*}
\includegraphics{drawings/ch6-servo.pdf}
