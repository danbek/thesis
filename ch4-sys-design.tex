...
The cutoff wavelengths of these filters are listed in \tableref{tab:xxx}.
...
This could be part of the explanation for the combination of poor optical efficiency and elliptical beams desribed in \chapterref{ch:imaging}.
...
But to simplify the analysis, I assume here that the light is unpolarized, so that the detectors are senstive to to uncorrelated waveguide modes.

The optical power per unit optical bandwidth in two uncorrelated modes detected by a detector with efficiency $\eta(\nu)$ is given by \cite{zmuidzinas_thermal_2003,richards_bolometers_1994}
\begin{equation} \label{eqn:ch4-power-per-mode}
  2 \eta(\nu) h \nu n_0(\nu),
\end{equation}
where $n_0$ is the photon occupation number give by the Bose-Einstein factor
\begin{equation} \label{eqn:ch4-photon-n}
  n_0(\nu) = \frac{1}{e^{\frac{h \nu}{k_B T}} -1}.
\end{equation}
For \SI{350}{\GHz} light emitted by a \SI{300}{\K}, $n_0 \approx 17$, so the Raleigh-Jeans limit $h \nu \ll k_B T$ holds, and \eqnref{eqn:ch4-power-per-mode} simplifies to
...
The photon NEP per unit bandwidth is given by \cite[Equation~51]{zmuidzinas_thermal_2003}
...
\caption{
  ...
  All \NEPph\ values are refferred to power absorbed in the bolometer.
  The total \NEPph\ is the quadrature sum of the contributions from all components, which is the correct sum to take for contributions from uncorrelated guassian noise sources.
  The efficiency of the lens accounts for losses due to both reflection from the lens and absrobtion in the lens.
...
\toprule 
  Component  & \specialcell{Temperature \\ (\si{\K})} & Efficiency & Emmisivity & \specialcell{$P_{opt}$ \\ (\si{\pW})} & \specialcell{\NEPph\ \\ (\si[mode=math,per-mode=symbol]{\femto\W\per\sqrt{Hz}})} & \specialcell{Cumulative \\ Efficiency} \\  
\midrule 
  Bolometer  &   1 & 0.87 &      &       &      & 0.87 \\ 
  Feedhorn   &   1 & 0.91 &      &       &      & 0.79 \\ 
  W1275      &   5 & 0.89 &      &       &      & 0.70 \\ 
  W1266      &  16 & 0.98 & 0.02 &   0.1 & 0.01 & 0.69 \\ 
  W1269      &  85 & 0.94 & 0.06 &   2.9 & 0.04 & 0.65 \\ 
  W1428      & 200 & 0.96 & 0.04 &   4.7 & 0.05 & 0.62 \\ 
  Lens       & 295 & 0.84 & 0.16 &  27.7 & 0.17 & 0.52 \\ 
  Beam       & 295 & 0.00 & 1.00 & 144.8 & 0.69 & 0.00 \\ 
\midrule 
  Total      &     &      &      & 180.2 & 0.72 & 0.52 \\ 
\bottomrule
\end{tabular}
\end{table*}
