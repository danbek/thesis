...
& & &  $\tan\delta = \num{4.0e-4}$ (\SI{9.1}{\percent} band-averaged absorbtion) \\
...
The absorption coeffienct for \HDPE\ at \SI{350}{\GHz} is $\alpha = \SI{0.044}{\cm^{-1}}$ \cite{lamb_miscellaneous_1996}, leading to \SI{8.5}{\percent} absorbtion in-band, due to the \SI{2}{\cm} thickness of the lens.

\section{Optical Filtering} \label{sec:ch4-filters}

Another important component of the system is the series of optical filters located inside the cryotat.
These filters are shown schematically in \figref{fig:ch4-filter-stack}.
These filters include a band-defining filter located at \SI{1}{\K}, thin thermal blocking filters located at \SI{300}{\K}, \SI{50}{\K}, and \SI{4}{\K}, and multilayer low-pass filters at \SI{50}{\K} and \SI{4}{\K}.
...
In this thesis I have chosen to described the filter as having a bandwith of \SI{35.0}{\GHz} with efficiency $35.0 / 31.1 = \SI{88.5}{\percent}$.

The thermal blocking filters are sheets of \SI{3.3}{\um} polypropylene with metal (capacative) grids printed on each side.
Because they are so thin, these filters have very little emission even at the infrared wavelengths at which polypropelene is highly absorbtive \cite{tucker_thermal_2006}.
...
This means that unless they are heatsunk very well, the centers of the filters will be much warmer than the stage at which they are anchored, and so they will re-radiate infrared power into the cryostat.
...
The outermost multi-layer filter is very warm (\SI{190}{\K}), and this is a significant source of loading on the \SI{4}{\K} stage, which also warms the \SI{4}{\K} multilayer filters, leading to a level of loading on the \SI{1}{\K} stage that is so high that the stage can not be brough below \SI{1.2}{\K} unless the aperture is stopped down to reduce IR loading.
...
\caption{
  Schemtatic showing locations of all filters in the \Imager.
  Everything shown here is internal to the cryostat.
  On the left the nominal tempertures for each stage are listed, and on the right the names and cuttoff wavelengths are listed.
}
...
\caption{
  ...
  ``Temperature'' gives the temperature at the center of a filter as measured by embedding a diode in a blob of Apiezon Type-N thermal grease placed on top of a layer of kapton tape, to prevent applying grease to the filter itself.
...
It allows for \SI{1}{\mil} spacing between the edges of the feedorns, and also accounts for a thermal conctaction factor of 4.14 parts per thousand \cite[Appendix~A6.4]{ekin_experimental_2006}.
...
The loss in \NETD\ from this suboptimal choice is only \SI{1.7}{\percent}.
...
An estimate of the insertion loss of the feedhorns requires the conductivty of this gold at \SI{1}{\K}. but this quantity is not known.
...
\HFSS\ simulations indate that the return loss for the feedhorns is \SI{-28}{\dB}, so I ignore return loss.
...
\end{equation}
where $eta_f \approx \SI{377}{\ohm}$ is the impedance of free space and $f_c$ the cuttoff frequency for the mode.
...
A design rule-of-thumb for these absorbing structures is that the effecive sheet impedance of the absorber is given by 
...
This rule-of-thumb has been justified via semi-empircal means \cite{ulrich_far-infrared_1967,whitbourn_equivalent-circuit_1985} for free-space absorbing grids.
...
Although the two polarizations have different absorption curves, with peak asorbtion at different frequencies, after averaging over the band their difference in efficiency is only \abt{\SI{1}{\percent}}.
...
For small misaligments (less than $\SI{100}{\um} \approx \SI{4}{\mils}$) the loss in coupling efficiency is small.
But for large misalignments the loss is efficiency will be large, and one mode will couple much worse than the other.
Becaues the beam for each individual mode is elliptical, this differential mode-coupling will lead to ellipitcal beams.
...
\caption{
  ...
  The left plot shows band-avereraged coupling efficiency vs misaligment, for misalignment in both $x$ and $y$ directions.
  The right plot shows the ratio of band-avereraged coupling efficiency of the $x$-polarized mode to the $y$-polarized mode, again for misalignment in both $x$ and $y$ directions.
  For misalignments up to \SI{100}{\um} loss in coupling efficiency is small.
  But for large misalignments the efficiency drops substantially, and the modes couples very differently, which will lead to a ellipitcal beams.
}
...
But to simplify the analysis, I assume here that the light is unpolarized, so that the detectors are senstive to two uncorrelated waveguide modes.
...
For each component \eqnref{eqn:ch4-power-per-mode} is integrated over the \Imager's optical band, the emmisivity factor from the table is applied uniformly over the band, and the transmission of all components after the given compoment is accounted for.
...
\caption{
  ...
  $P_{opt}$ for each compoment is calculated according to \eqnref{eqn:ch4-opt-pow-all}.
  $\epsilon \eta n$ is calculated at the the center frequency of the band (\SI{347}{\GHz}) and also includs the tranmission of the bandpass filter at that frequency.
  xxx what about bandpass filter?
  All powers are the power absorbed in the bolometer, and the \NEPph\ value is refferred to power absorbed in the bolometer.
  The efficiency of the lens accounts for losses due to both reflection from the lens and absorbtion in the lens.
...
A \SQUID\ is a Superconducting Quantum Interferance device \cite{clarke_squid_2002}.
...
To linearize the \SQUID\ ampifier chain, feedback in the form of magnetic flux is applied to the 1st-stage \SQUIDs.
...
The \Imager uses the Multi-Channel Electroncs (\MCE) as the electronics control system \cite{battistelli_functional_2008,battistelli_automated_2008,_mcewiki_2014}.
The \MCE\ was developed in order to achieve simple, remote operation of \TDM\ readout systems for sub-millimeter nad millimeter astronomy.
...
The \MCE has many parameters that can be set to control its behavior, almost none of which I will decribed here.
...
This noise aliasing penalty can be reduced by configuring the \MCE\ to interally apply a digital 4-pole lowpass filter prior to reporting data to the data acquisition computer \cite{mce_team_digital_????}.
