...
This system serves as a demonstration that practical passive video imaging can be achieved at these wavelengths through the use of large-format arrays of cryogenic detectors
...
It will also serve as a test-bed for evaluating new capabilities such as polarization and spectroscopy.
...
These include detection of suicide bomb belts, crowd surveillance or imaging through fog or dust, all scenarios in which is it either desirable or required to make a detection while the object being observed is some distance away.
...
The angular resolution $\theta$ achievable by an aperture of diameter $D$ observing at wavelength $\lambda$ is, by the Rayleigh criterion \cite{born_principles_1999},
...
Passive system have the advantages that they are inherently covert, and do not suffer from public resistance due to worries about the health effects of illumination by millimeter-wavelength ``radiation''.
...
If the detector is sensitive to all light incident from all directions, $M = 2 \pi \nu^2 / c^2$, where $c$ is the speed of light, and the result is Planck's law.
