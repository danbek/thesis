...
The measured L values include two sets of clear outliers.
All values for row 4 are clustered around 200~nH, and all values for row xxx are clustered around 425~nH (xxx check against means).
The reason for low inductances for these rows is not understood.
These outliers are more clearly visible in the rightmost plot in \figref{fig:rsh-l-plots}, which also shows a small correlation between Rsh and L.
It is not understood whether this is a real physical effect, or whether it is an artifact of the measurement process.

\begin{figure*}
\input{\here/rsh-l-plots.tex}
\caption{Plots summary results of measurements of shunts and Nyqust inductors.
\textbf{Left} Histogram of shunt resistance \Rsh.
\textbf{Middle} Histogram of total inductance in circuit, which includes the interface chip Nyquist inductor, the inductance of the SQ1 input coil, and any parasitic inductance.
\textbf{Right} Scatterplot showing all measured Rsh and L values. A correlation is clearly visible, the explanation for which is not understood.}
