...
\begin{equation}\ref{eqn:tes-ptot}
...
\textbf{Lower Left} Measured $P_{J}$ vs heater current at 99\% $R_n$, as well as fit to \eqnref{eqn:rhtr-fit}.
...
I again used the assumption that high in the transition the \TES\ resistance depends on only the \TES\ temperature, so that the Joule power $P_J$ dissipated in the \TES\ at 99 \% $R_n$ depends only on the bath temperature $T_b$ and the applied heater power $P_{htr} = I_{htr}^2 R_{htr}$. A fit can then be made to \eqnref{eqn:tes-ptot} in the form
