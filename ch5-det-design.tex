...
The criterion used here is to require require the \FWHM\ of the timestream bump after blurring to be no more than twice that of the unblurred bump.
...
$\tau_{eff}$ in the prototype detectors was \abt{xxx ms}, so for unblurred imaging at 20 frames per second, $\tau_{eff}$ should be reduced by a factor of xxx from the prototype detectors.
As long as \Loop\ and $\beta_I$ at the operating bias point did not change from the prototype detectors, and given the design values of $G$, this would reduced a reduction in the heat capacity $C$ of the detector by a factor of xxx.
To reduce the risk of problems such as instability with the sub-array detectors, I decided to reduce $C$ to \SI{30}{\pJ\per\K} by cutting the thickness of the Au ring in half, which would give $\tau = \SI{8}{\ms}$, and $\tau_{eff} = xxx$.
