...
\begin{itemize}
  ...
  \item The saturation power of the \TES\ detector scales roughly like $G T_c$, so that if $G$ and $T_c$ are too small, the optical power falling on the detector will raise the temperature of the membrane above $T_c$, causing the device not to work.
...
\caption[Photograph of focal plane in cryostat]{
  ...
  Also visible in the photograph are the end-points of the Cu ropes that connect the \PTC\ 2nd-stage cold head to the \SI{80}{\K} plate (\textbf{B}).
}
...
Four 100-wire woven PhBr wire harnesses run from \SI{300}{\K} to the focal plane, heat sunk along the way at the \SI{80}{\K} and \SI{6}{\K} stages.
