...
The overall system size are set by the standoff distance and desired spatial resolution, which directly determine the size of the optical aperture via \eqnref{eqn:ch1-raleigh}.
...
Operating at higher bath temperatures can reduce cost if the new bath temperature can be achieved without the use of a secondary cooling system such as the \HE4-sorption refridgerator used for the \Imager.
...
As mentioned in \sectionref{xxx}, one group is already working on a passive imaging system that will use these detectors.
