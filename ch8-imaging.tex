...
% xxx should update all plots with latest fudge factor of 1.1457

This chapter descibes the process of turning detector timesteams into videos, as well as measurements required to support that process.
...
The direct measurements of distances in the far-field described in \sectionref{sec:ch8-dist-scale} show that $F_d = 1.15$, and this value is used in all analysis for the remainder of this chapter.
...
The result of these measurements is that $F_d = 1.15$.
...
The total temperature contrast in the images of \figref{fig:ch8-single-frame} is \SI{6}{\K}, but as described above higher and low values of temperature have been mapped to \SI{+3}{\K} and \SI{-3}{\K} to improve contrast for display.
...
\end{itemize}
If the coldest temperature in the images is given by the room temperature, then the total contrast in the image is preditected as
\begin{equation} \label{eqn:ch8-pred-contrast}
  0.825 \times \SI{307}{\K} + (1 - 0.825) \times \SI{295}{\K} - \SI{295}{\K} =  \SI{9.9}{\K},
\end{equation}
about \SI{1.8}{\K} larger than observed.
...
The total contrast in the raw images is \SI{8.2}{\K}, about \SI{1.8}{\K} lower than expected.
...
\begin{figure*}[th]
\centering
\includegraphics[width=\textwidth]{drawings/ch8-beams-on-image.pdf}
\caption{
  Plot showing where beams are pointed in farfield.
...
If this is not true for a particular detector, then then ellipse traced out by that detector in the image will appear warmer or cooler than its surroundings (depending whether the distibution that it viewed was cooler or warmer than it's surroundings), leading to ellpitical scanning artifacts.
...
It should be possible to take advantage of this to set the offets for each detector correctly on a frame-by-frame basis.
...
This process could be repeated several times, untill a self-consistent image is created.
...
\item The total integration time per pixel is not uniform.
      Some pixels end up receiving much more detector samples then others, as shown in \figref{fig:ch8-flat-frame-cnt}.
...
\begin{figure*}[th]
\centering
\includegraphics[width=\textwidth]{drawings/ch8-flat-frame-cnt.pdf}
\caption{
  Distibution of samples per pixel for flat field image shown in \figref{fig:ch8-flat-netd}.
  Different video frames have slightly different distibutions, but the overall features are all similar to what is shown here.
  The bins in the histogram on the right are one sample wide, so that it can be seen that a handful of pixles around the outer edge recieve only one sample.
}
