...
Inside the cryostat two \SI{1}{m} long Phosphur Bronze (PhBr) \AWG36 twisted pair wires carry the signal to the focal plane, where the signal is fed into the input coil of a 1st stage \SQUID.
% xxx get exact lakeshore name for ph-br 
...
To convert actuator displacement to displacements in the far-field of the system, I used the conversion factors produced from \ZEMAX\ listed in \sectionref{sec:ch4-optical-design}.
...
The direct measurements of distances in the far-field described in \sectionref{sec:ch8-dist-scale} show that $F_d = 1.16$, and this value is used in all analysis for the remainder of this chapter.
...
As described in \sectionref{sec:ch4-optical-design}, the \Imager\ is designed to focus at distances of 16~m -- 30~m.
% (xxx check 2nd distance).
...
\figref{fig:ch8-beam-vs-distance} shows beam maps for the same detector taken at different distances.
...
Possibilities include the cryostat being located too close or errors in the manufacturing or assembly of the optical components.

% xxx need to add fig:ch8-beam-vs-distance figure!

\section{Beam Maps} \label{sec:ch8-beam-maps}

As discussed in \sectionref{sec:ch4-feedhorn-design}, the \Imager\ feedhorns are predicted to have beams that are circularly symmetric and well-approximated by Gaussians with \FWHM\ of xxx~cm at the target.
...
The black-body source used was an IR Labs xxx\footnote{IRLabs, Inc. Tucson, AZ. USA}.
This source reaches a maximum of \SI{1030}{\celsius} and has apertures ranging in size from xxx in to xxx in.
...
\figref{xxx} shows a picture of the black-body source with Aluminum foil mask.
...
% xxx remove the two boring scatter plots, make 2x2 grid
\begin{figure*}[th]
\centering
...
A sheet of \ecco\footnote{xxx need company ref} was placed in the cooler to serve as a cold black surface for the system to observe.
...
% xxx include a plot explaining this idea.
...
However, I do allow for a non-unity transmitivity $\tau$ for the Styrofoam in the analysis below.
% xxx - transmitivity spelling
...
I assume that the transmitivity takes the form $e^{-d \kappa}$, where $d$ is the thickness of the material and $\kappa$ is some constant which characterizes the attenuation length of \SI{350}{\GHz} light in Styrofoam.
Given this assumption, if the transmitivity through the cooler's wall is $\tau$, then the transmitivity through the lid is given by $\tau^{2/1.625} = \tau^{1.23}$.

The optical powers viewed under these three conditions is then given by (with $T_h = \SI{295}{\K}$ and $T_c = \SI{77}{\K}$)
% xxx make nice multiline equations with alignment by the = sign. And
% need to make clear is is power in 2 optical modes *emitted* at source
\begin{equation}
...
The transmitivity $\tau$ of the cooler wall was found to be \abt{\SI{90}{\percent}} and the optical efficiency $\eta_{tot}$ was \SI{13.6}{\percent}. 
...
%It is difficult to interpret this measurement because it is not clear what temperature distribution the detectors are now seeing, in particular whether the foil is reflecting a different amount of optical power from inside the cryostat onto the detectors.
...
To convert this to temperatures changes in the far-field of the system, we must use the total optical efficiency $\eta_{tot}$, as well as the Raleigh-Jeans limit of the optical power per spatial mode from a temperature distribution (see \sectionref{sec:ch4-opt-eff}).
  ...
  \item Perform the following processing for each detector that is not on the cut list described in \sectionref{sec:ch8-det-cuts}:
    ...
    \item If the detector's timestream shows evidence of glitches, do not include that detector's timestream for this video frame. The algorithm used to identify glitches is the following:
      ...
      \item If the absolute value of any $\Delta f_j$ is greater than five times the standard deviation, identify this detector as having a glitch, so that it will be ignored for this video frame.
            % xxx should say how often this happens, show example of glitch timestream ... maybe show a big plot with all timestreams for a frame, highlighting those with glitches?
      \end{enumerate}
...
% xxx add commentary on offset algorithm
...
\item The total integration time per pixel is not uniform. Some pixels end up receiving much more detector samples then others.% (xxx quantify)
...
% However, as shown in \sectionref{sec:ch7-det-noise}, the detector noise is not white.
...
% \end{equation}
% $N$ is the number of samples in the timestream, and $f_s$ is the sampling frequency.
...
% This should cause the \abt{\SI{18.5}{\fW}} detector noise standard deviation to decrease by $1/\sqrt{6.7}$ to \SI{6.6}{\fW}.
