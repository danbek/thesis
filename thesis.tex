...
% xxx in general in this section I am not being clear about whether
% I'm discussing the feedhorn in transmission or reception. It might
% be a good idea to clarify this issue early on, and then stick to one
% or another.

% xxx would it be better to discuss number of feedhorns in terms of
% number per side in a square grid?

% xxx where do I explain the choice of a square grid?
...
\figref{fig:feedhorn-parms} shows a schematic of a smooth-walled conical feedhorn, its beam and the beam's interaction with the primary mirror.
As discussed in section \sectionref{s:optical-design}, although the optical system has a secondary mirror as well, for many purposes the secondary mirror can be ignored and the system treated as a system of feedhorns illuminating only the primary mirror.
The ``beam'' $P(\theta, \phi)$ of a feedhorn refers to the pattern of radiation that the beam is senstive to.
If a feedhorn is observing a radiation pattern with temperture $T_{target}(\theta,\phi)$, and it pointed in a direction $(\theta_0, \phi_0)$, then the temperature observed by the feedhorn will be
\[
    T_{eff} = \int \, d \Omega \, T_{target}(\theta - \theta_0,\phi - \phi_0) P(\theta,\phi)
\].
When using this expression one must keep in mind that the fraction of the beam that spills off the primary mirror (shaded in \figref{fig:feedhorn-parms}) will see not the temperature distribution of the target, but a temperature distribution determined by what is beyond the primary mirror\footnote{Because of the presence of the secondary mirror, some of this tempeature distribution will be in front of the system, and some will be behind.}.
...
\begin{figure*}
\centering
\input{drawings/ch4-feedhorn-parms.tex}
...
It is more helpful to think of the feedhorn diameter as a value that can take on a discrete set of values that depends on the number of feedhorns per each side of the grid.
...
\]
Here $E_{\theta}$ and $E_{\phi}$ are functions depending the horn diameter $D$ and opening half-angle $\theta_h$, and involving definite integrals of Bessel functions, not reproduced here.
...
To calculate the spillover efficiency of an individual feedhorn, the \MATLAB program integrates $P$ over the angles $\theta$ that illuminate the primary mirror and reach the target: 5.3\textdegree to $\theta = $ 13.6\textdegree\footnote{see \sectionref{s:optical-design}}.
\figref{fig:spill-vs-alpha-diam} shows a contour plot of spillover efficiency as a function of $D$ and $\theta_h$.
The blue asterisk matches the parameters for the feedhorns chosen for the \Imager.
This plot makes it clear that $\eta_s$ depends much more strongly on $D$ than on $\theta_h$.
$\theat_h = 9.4$\textdegree was chosen as a value that is easy to machine, keeps the thermal mass of the feedhorn array relatively low (small values of $\theta_h$ lead to long feedhorns and higher thermal mass), and it not too far from the maximum achievable $\eta_s$ for any fixed feedhorn diameter $D$.

\begin{figure*}[th]
\centering
\includegraphics[width=3.0in]{images/spill_cs_alpha_diam.pdf}
\caption{Plot showing feedhorn spillover efficiency $\eta_s$ as a function of horn diameter $D$ and horn flare half-angle $\theta_h$. The blue asterisk shows the feedhorn parameters used in the \Imager. xxx need to replace with version that has * sign at real feedhorn values!}
...
In fact, as \figref{xxx} shows, an even larger number of even smaller feedhorns is predicted to reduce \NETD by xxx \%.
...
\caption{Plot showing how total number of detectors and system \NETD depend on diameter chosen for feedhorns. The number of detectors is a discrete function of the feedhorn diameter because it is not possible to have a fractional part of a feedhorn. \NETD is maximed with xxx feedhorns of diameter xxx, but the \Imager uses 1004 feedhorns of diameter xxx because of readout limitations. xxx Is this plot right? I chose 2.68 mm (cold), which leads to ~800 detectors, not 1024.}
\label{fig:spill-vs-alpha-diam}
\end{figure*}
