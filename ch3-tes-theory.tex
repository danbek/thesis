...
At each bias point the \DC\ change in \TES\ current $\delta I$ and $\tau_{eff}$ is measured by fitting the \TES\ response to \eqnref{eqn:htr-step-resp-high}, and the bias current $I_{bias}$ is recorded.
...
Under this assumption, each time the \TES\ resistance reaches, e.g.\ $R = 0.99R_n$, the temperature of the \TES\ is the same, and therefore $P_{tot}$ will be the same.
...
\begin{align}
  P_{sat} & \equiv K(T_c^2 - T_n^2) \\
         & = \frac{G T_c}{n} \left( 1 - \left(\frac{T_b}{T_c}\right) \right).
 \label{eqn:ch3-psat}
\end{align}
