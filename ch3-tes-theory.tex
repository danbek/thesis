...
\figref{fig:ch3-g-plots} shows an example of this fit for \RCm{31}{2}.
...
Addionally intrinsic fluctuations in the number of arriving photons leads to photon noise, which can be significant source of noise for some \TES\ bolometers.
...
Photon noise arrises due to quantum fluctuations in the number of photons arriving during a given time interval.
xxx perhaps start with Zmuidzinas 44 with $(hv)^2$ added for bolometers, then $\bar{n} = k_B T / h \nu$ leading to
This noise is expressed as
\begin{equation}
...
\begin{table*}[t]
\centering
\caption{Noise in \TES\ bolometers, referred to power incident on bolometer. To obtain current noise passing through the bolometer, multiply each power spectral density by $|s_I(\omega)|^2$.}
\label{tab:tes-noise}
\begin{tabular}{l l}
\toprule
Noise Source &  Current Noise \\
\midrule
\TES\ Resistor & $S_{I_{TES}} = 4 k_B T_0 I_0^2 R_0 \xi(I_0) \frac{(1 + (\omega \tau)^2) } {\Loop^2}$ \\
Load Resistor & $S_{I_{L}} = 4 k_B T_L I_0^2 R_L \frac{(1 + (\omega \tau_I)^2) (\Loop -1)^2} {\Loop^2}$ \\
Thermal Fluctuation Noise & $S_{TFN} = 4 k_B T_0^2 G F(T_0, T_b)$ \\
\bottomrule
\end{tabular}
\end{table*}
