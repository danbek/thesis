...
In order to turn detector outputs into an image, we need to know the detector power-to-current responsivity $S_I(0)$ and the total optical efficiency.
This chapter presents a series of detector measurements culminating in predictions of $S_I(0)$ for all working detectors, while \chapterref{c:imaging} presents measurements of optical efficiency.
...
The average value of \SI{9.6}{\ms} is \abt{\SI{20}{\percent}} slower than the design value of \SI{7.9}{\ms}, which would worsen video image blurring as discussed in \sectionref{sec:det-parm-choice}, though not by a large amount.
xxx discuss how estimation of teff means that in pratice this is not all that bad?
