...
\figref{fig:ch6-bias-step-plots} and \figref{fig:ch6-bias-step-results} show the results of these measurements for the four detectors on column 0 with good heaters.
...
\caption[Predictions for $\tau_{eff}$ and $s_I(0)$]{
  Plots showing distribution of predicted $\tau_{eff}$ and $s_I(0)$. The predictions use the values for $R$, \Loop and $\beta_I$ shown in \figref{fig:ch6-all-loop-plots}, and $R_{sh}$ values from \sectionref{sec:shunt-nyquist}. $R_p$ is assumed to be zero in all cases.
}
...
\figref{fig:ch6-cm-plots} contains plots relating to this.
...
\figref{fig:ch6-ptc-plots} shows a histogram of this excess noise. Most detectors have higher noise with the \PTC\ on, but this excess noise is typically only a few percent. 
...
5 data files were acquired for each case. For each row in column 0 and for each data file a power spectrum was taken after subtracting the common mode signal, and the excess noise was calculated as
\begin{equation}
...
While taking the \Loop abd $\beta_I$ measurements described in \sectionref{sec:bias-step}, I also took noise data for the four detectors with working heaters on column 0.
...
Finally, \figref{fig:ch6-all-noise} shows histograms of the total noise in each detector using three different values for the servo gain $I_{FB}$: 50, 20, and 10.
